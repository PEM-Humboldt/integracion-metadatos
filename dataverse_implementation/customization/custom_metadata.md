...
Hasta el momento, Dataverse no cuenta con el soporte para modificar los campos de esta sección, pues no pueden ser agregados ni modificados a partir de archivos TSV como los demás metadatos. Tampoco es posible generar una comunicación entre un metadato y un campo de las condiciones (que el cambio en un campo active un cambio en el otro), ya que no existe este soporte nativo en el aplicativo. 
