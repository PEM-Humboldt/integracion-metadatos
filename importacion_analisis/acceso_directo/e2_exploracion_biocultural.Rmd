...
tableAttrib$datafile <- rbind(
  dataframe(id=NA,metadatablock=NA,name="datafile_id",sql_name="createtime",type="timestamp",primarykey=F,reference=NA)
```
...
createTableStatement<-function(nameTable,tabAttr,dbConnection)
{
  ...
  if(class(dbConnection)=="SQLiteConnection")
  {
    ...
    tabAttr$type <- c(int="integer", bigint="integer", smallint="integer", integer="integer", serial="integer", bigserial="integer", smallserial="integer", date="text", text="text", real="real", `double precision`="real")[tabAttr$type]
...
```{r}
extractMetaData <- tableAttrib[ ! names(tableAttrib) %in% c("dataverse","dataset","datasetversion") ]
extracted<-vector(mode="list",length=length(extractMetaData))
for(i in 1:length(extractMetaData))
{
...
names(extracted)<-names(extractMetaData)
...
save_in_excel(exportXl_file,extracted)
```

### Dataset information from direct dataverse database
