...
- [4 Exportation of a table with values and references (dataset and
  ...
  - [4.1 Crear la base de datos](#41-crear-la-base-de-datos)
  - [4.2 Crear las primeras tablas](#42-crear-las-primeras-tablas)
    - [4.2.1 Información de grupos de
      variables](#421-información-de-grupos-de-variables)
  - [4.3 Crear the metadata tables](#43-crear-the-metadata-tables)
  - [4.4 Particularidad del IPT: registro de
    archivos](#44-particularidad-del-ipt-registro-de-archivos)
- [5 Export finalTables Excel](#5-export-finaltables-excel)
- [6 Apagar la luz y salir](#6-apagar-la-luz-y-salir)
...
for (i in 1:length(finalTables)) {
    if (is.na(foreignKey[i])) {
        ...
        preamble <- data.frame(primaryKey = 1:nrow(matData[[i]]), foreignKey = foreignKeyMatch[[i]])
        names(preamble) <- c(primaryKey[i], foreignKey[i])
    }
    finalTables[[i]] <- cbind(preamble, lapply(as.data.frame(matData[[i]]),
...
## 4.2 Crear las primeras tablas

### 4.2.1 Información de grupos de variables

For now we will only create the tables gp_info, var_info and
sub_var_info, to describe the way final tables and var were managed in
the xml files:

``` r
statement <- "CREATE TABLE gp_info(
...
``` r
statement <- "CREATE TABLE var_info(
...
``` r
statement <- "CREATE TABLE subvar_info
(
...
``` r
tab0 <- data.frame(refUn = 0, name = "dataset", varPath = "", inGp = NA,
    gp_path = "gp_0")
...
dbAppendTable(metaCeiba, Id(table = "subvar_info"), data.frame(cd_subvar = 1:length(unlist(sp)),
    cd_var = rep(var_info$refUn, sapply(sp, length)), subname = unlist(sp)))
```

    ## [1] 21

## 4.3 Crear the metadata tables
...
for (i in 1:length(finalTables)) {
    ...
    if (!is.na(foreignKey[i])) {
        ...
        fk[w] <- paste0(" REFERENCES ", dbQuoteIdentifier(metaCeiba, Id(table = foreignTables[i])),
            "(", dbQuoteIdentifier(metaCeiba, foreignKey[i]), ")")
    }
    dbExecute(metaCeiba, paste0("CREATE TABLE ", dbQuoteIdentifier(metaCeiba,
        Id(prepTables[[i]]$nameTable)), "(", paste(quotedCol, types, pk,
        fk, collapse = ","), ")"))
    if (!is.na(foreignKey[i])) {
        dbExecute(metaCeiba, paste0("CREATE INDEX fk_", prepTables[[i]]$nameTable,
            "_", foreignKey[i], "_", foreignTables[i], "_idx ON ", dbQuoteIdentifier(metaCeiba,
                Id(table = prepTables[[i]]$nameTable)), " (", dbQuoteIdentifier(metaCeiba,
                foreignKey[i]), ");"))
    }
    dbAppendTable(metaCeiba, dbQuoteIdentifier(metaCeiba, Id(prepTables[[i]]$nameTable)),
        finalTables[[i]])
}
```

## 4.4 Particularidad del IPT: registro de archivos
...
``` r
dataFiles <- data.frame(cd_file = 1:length(result_find), root_folder = sapply(strsplit(result_find,
    "/"), function(x) x[baseLength + 1]), file = basename(result_find),
...
if (!"cd_file" %in% dbListFields(metaCeiba, "dataset")) {
    ...
    dbWriteTable(metaCeiba, "tmp_corres_dataset_file", data.frame(cd_dataset = 1:nrow(addresses_xml),
...
```
    ...
    ## Warning in wb$writeData(df = x, colNames = colNames, sheet = sheet, startCol = startCol, : ), Tiempo atmosf ... to del Casanare. is truncated. 
    ## Number of characters exeed the limit of 32767.

# 6 Apagar la luz y salir
