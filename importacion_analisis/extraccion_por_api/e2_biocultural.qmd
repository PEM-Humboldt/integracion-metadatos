...
Describir las variables de metadatos de Biocultural (obtenidos por API) de la misma forma que lo que se hizo para los metadatos de Ceiba y Geonetwork nos permitiría hacer un trabajo de integración, desde bases comparables en los 3 catálogos.

Parece que, probablemente por los elementos de la lista enviada en `JSON` por la API que no contienen nombres de elementos, no se pueden aplicar las funciones que se aplicaron sobre los metadatos en `XML` de los catalogos Geonetwork y Ceiba.
Por ahora, podría ser más eficiente trabajar los metadatos y su estructura desde la base de datos en PostgreSQL del catálogo Biocultural
